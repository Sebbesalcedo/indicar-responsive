...
export class LoginDialog{
    ...
    switch(this.view){
      ...
      case 'register':
        ...
        this.formRegister2 = new FormGroup({
          ...
          flname:     new FormControl('',[Validators.required, Validators.maxLength(100)]),
          fphone1:     new FormControl('',[Validators.required, Validators.maxLength(15)]),
          ...
          frazonsocial: new FormControl('',[Validators.required, Validators.maxLength(150)]),
          fnit:       new FormControl('',[Validators.required, Validators.maxLength(20)]),
          faddress:   new FormControl('',[Validators.required, Validators.maxLength(200)]),
          fphone2:     new FormControl('',[Validators.required, Validators.maxLength(15)]),
        ...
        this.AutheticationService.checkEmail(event.target.value)
          .subscribe(
            data =>{
              ...
              datos = data;
              if(datos.cantidad == 1){
                this.emailInvalid = "E-mail ya registrado por favor inicie sesión";
              }
            },
            error=>{
              console.log(error);
  ...
  onSubmitLogin(register:boolean = null){
    ...
    this.AutheticationService.login(username,password)
    .subscribe(
      data =>{
        let datos;
        datos = data;
        if(datos.hasOwnProperty('success')){
          if(datos.success == 'true'){
            ...
            if(setFavorite != null){    // establecer clasificado como favorito.
  ...
  onSubmitRegister(){
    ...
    if (!this.formRegister2.valid){
      ...
      let pass,cpass,phone,term,fname,sname,flname,slname,direccion,nit,acepto;
      if(this.isConcesionario == "01"){  // persona natural
        pass    ='fpass1';
        cpass   ='fcpass1';
        phone   ='fphone1';
        term    ='fterm1';
        ...
        if(sname!= undefined){
          sname   = sname.slice(0,1).toUpperCase()+sname.slice(1).toLowerCase();
        }
        ...
        if(slname!= undefined){
          slname   = slname.slice(0,1).toUpperCase()+slname.slice(1).toLowerCase();
        }
      ...
      if( this.formRegister2.get(pass).value!=this.formRegister2.get(cpass).value){
      ...
      this.AutheticationService.register(data)
      .subscribe(
        result =>{
          this.AutheticationService.login(data.email,data.password)
          .subscribe(
            response =>{
              let datos;
              datos = response;
   ...
   */
  onSubmitRecovery(){
    if(this.formRecoveryPassword.get('fpass').value != this.formRecoveryPassword.get('fcpass').value){
      swal.fire({
        title: '',
        icon: null,
