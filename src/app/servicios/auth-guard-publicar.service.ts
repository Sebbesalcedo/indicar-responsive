...
export class AuthGuardPublicarService implements CanActivate{
  ...
  private secureLogin() {
    ...
    if (this.isLogged) {
      ...
      if (nsr == null || nsr == "null") {
        // si distinto de null no se reporta ningun evento de ingreso.
        if (ieuf != null && ieuf != "null" && ieuf != undefined) {
          // existen tokens.
          if (Array.isArray(ieuf)) {
            let execute = true;
            ieuf.forEach((item, i) => {
              // token encontrado en la lista. se debe comprobar el token.
              if (
                item.username.toLowerCase() == usuario.username.toLowerCase()
              ) {
            ...
            if (execute) {
              // creamos token. ya que este usuario no existe.
              this.createToken();
            }
          }
        } else {
          // no existen tokens. deben crearse y reportar accesos.
          this.createToken();
        }
      } else {
        // /nsr
  ...
  private createToken() {
    ...
    let tk = btoa(
      this.cuser.username +
        "eyJwYWdlIjoiaW5kaWNhciIsInRva2VuIjoiSW5kaWNhcjIwMTlFcXVpcG9EZVVzb0ZyZWN1ZW50ZSJ9" +
        timeTk
    ); // encode de token con correo.
    if (ieuf == null) {
      this.ieuf.push({
      ...
      // mantengo los registros existentes y agrego los nuevos.
      ieuf.push({
        username: this.cuser.username.toLowerCase(),
        token: tk,
      });
  ...
  private comprobarToken(token: any, idToken: number, action: string) {
    ...
    if (action == "CREATE_TOKEN") {
      // creo token en DB. y envio email reportando acceso.
      body = {
        token: token[idToken],
      };
      // REQUEST
  ...
  sendRequestSearch(router, search) {
    router.navigate(["/clasificados"], {
      queryParams: { p_descripcion: search },
    });
  }
  /**
